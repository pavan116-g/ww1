$filePath = "c:\Users\pavan\ww1\src\components\PreWW1.jsx"
$lines = Get-Content $filePath
$head = $lines[0..437]
$tail = $lines[667..($lines.Count - 1)]
                                ...
                                <div style={{ width: '260px', flexShrink: 0, borderRight: `1px solid ${borderColor}`, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: `${accent}33 transparent` }}>
                                    <div style={{ padding: '0.9rem 1.2rem 0.6rem', borderBottom: `1px solid ${accent}18` }}>
                                        <div style={{ fontSize: '0.58rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: accent, fontWeight: 700, opacity: 0.65 }}>
                                            {activeFront === 'west' ? 'Western Front' : activeFront === 'east' ? 'Eastern Front' : activeFront === 'middle-east' ? 'Middle East' : activeFront === 'naval' ? 'Naval Warfare' : 'Italian Front'} &middot; {battles.length} Battles
                                        </div>
                                    </div>
                                        ...
                                        {(() => {
                                            ...
                                            return (
                                                <div style={{ width: '270px', flexShrink: 0, padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '1.2rem' }}>
                                                    <div style={{ fontSize: '0.5rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.28)', fontWeight: 700 }}>Tug of War &middot; Momentum</div>
                                                    <div>
...
$newLines = $newSection -split "`r?`n"
$all = $head + $newLines + $tail
Set-Content $filePath -Value $all -Encoding UTF8
Write-Host "Done! Total lines: $($all.Count)"
